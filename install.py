...
<div id='api' class='view'><div class='section-head'><div><h1 class='section-title'>API 模型</h1><div class='hint'>OpenAI 兼容接口或中转站。密钥只保存到本机加密文件。</div></div><button class='btn primary' onclick='showProviderForm("api")'>新增 Provider</button></div><div id='api-list' class='provider-list'></div><div id='api-form'></div><div class='card' style='margin-top:14px'><h2 class='section-title' style='font-size:18px'>协议路由</h2><p class='hint'>消息格式不是 Chat 的 API 模型由内置路由做协议转换（Responses/Anthropic ↔ Chat）。默认端口 15731，与本机 cc-switch 的 15721 错开；被占用时可改。</p><div class='form'><label class='field'><span>路由端口</span><input id='router-port' type='number' min='1024' max='65535' value='15731'></label></div><div class='toolbar'><button class='btn primary' onclick='saveRouter()'>保存端口</button></div></div></div>
...
async function saveRouter(){try{const d=await api('/api/settings',{method:'POST',body:JSON.stringify({api_router:{port:Number($('router-port').value)||15731}})});state=d.config;render();toast('路由端口已保存')}catch(e){toast(e.message)}}
...
async function testProvider(kind,id){try{const d=await api('/api/test',{method:'POST',body:JSON.stringify({kind,id})});toast(d.message)}catch(e){toast(e.message)}}async function testApiForm(){const endpoint=$('f-endpoint').value,model=$('f-model').value,key=$('f-key').value,fmt=($('f-format')||{}).value||'chat';try{const d=await api('/api/test',{method:'POST',body:JSON.stringify({kind:'api',endpoint,model,api_key:key,format:fmt})});toast(d.message)}catch(e){toast(e.message)}}
