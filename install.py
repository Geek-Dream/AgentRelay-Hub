...
<div id='commander' class='view'><div class='section-head'><div><h1 class='section-title'>Commander</h1><div class='hint'>主 Agent 负责分工、审查和合并，子 Agent 最多 5 个。</div></div><button class='btn primary' onclick='saveCommander()'>保存</button></div><div class='card'><div class='form'><label class='field full check'><input id='commander-enabled' type='checkbox'> 开启审查官模式</label><label class='field'><span>使用的终端</span><input id='terminal' value='codex'></label><label class='field'><span>子 Agent 启动命令</span><input id='command' value='codex exec'></label><label class='field'><span>最多同时运行</span><input id='max-agents' type='number' min='1' max='5' value='5'></label></div><p class='hint'>DeepSeek 和本地模型只是辅助工具；API Provider 不会交给 Commander 子 Agent。没有额外模型时仍可用当前 Codex CLI。</p></div></div>
...
function render(){const c=state.commander||{};$('mode-pill').textContent=c.enabled?'Commander 高级模式':'离线基础模式';$('config-path').textContent=state.config_path||'本机加密目录';$('commander-enabled').checked=!!c.enabled;$('terminal').value=c.terminal||'codex';$('command').value=c.command||'codex exec';$('max-agents').value=c.max_agents||5;const counts=allProviders().map(x=>x[1].length);$('stats').innerHTML=[['线上 Provider',counts[0]],['本地 Provider',counts[1]],['API Provider',counts[2]]].map(x=>`<div class='card'><div class='label'>${x[0]}</div><div class='stat'>${x[1]}</div></div>`).join('');$('overview-providers').innerHTML=allProviders().flatMap(([k,items])=>items.map(p=>providerCard(k,p,false))).join('')||`<div class='empty'>还没有 Provider，离线基础能力仍可使用。</div>`;['web','local','api'].forEach(k=>$(k+'-list').innerHTML=(state[k+'_providers']||[]).map(p=>providerCard(k,p,true)).join('')||`<div class='empty'>暂无配置，点击右上角新增。</div>`)}
...
function showProviderForm(kind,id=''){const p=(state[kind+'_providers']||[]).find(x=>x.id===id)||{};const c=p.conversation||{};const titles=c.titles||{};let body='';if(kind==='web')body=`<div class='card'><h2 class='section-title' style='font-size:18px'>${id?'编辑':'新增'}线上模型</h2><div class='form'><label class='field'><span>别名</span><input id='f-id' value='${esc(p.id)}' placeholder='deepseek-web'></label><label class='field'><span>显示名称</span><input id='f-name' value='${esc(p.name)}' placeholder='DeepSeek'></label><label class='field full'><span>对话页面网址</span><input id='f-url' value='${esc(p.base_url||p.url)}' placeholder='https://chat.deepseek.com/'></label><label class='field check'><input id='f-flash' type='checkbox' ${c.supports_flash!==false?'checked':''}> 支持极速模式</label><label class='field check'><input id='f-expert' type='checkbox' ${c.supports_expert!==false?'checked':''}> 支持专家模式</label><label class='field check'><input id='f-hybrid' type='checkbox' ${c.supports_hybrid!==false?'checked':''}> 混合模型</label><label class='field check'><input id='f-images' type='checkbox' ${c.supports_images?'checked':''}> 支持图片</label><label class='field check'><input id='f-create' type='checkbox' ${c.create_if_missing!==false?'checked':''}> 没有会话时自动创建</label><label class='field'><span>极速会话名称</span><input id='f-flash-title' value='${esc(titles.flash||'AgentRelay-DeepSeek-Flash')}'></label><label class='field'><span>专家会话名称</span><input id='f-expert-title' value='${esc(titles.expert||'AgentRelay-DeepSeek-Expert')}'></label><label class='field'><span>混合会话名称</span><input id='f-hybrid-title' value='${esc(titles.hybrid||'AgentRelay-DeepSeek')}'></label></div><div class='toolbar'><button class='btn' onclick='loginWeb()'>打开网页登录</button><button class='btn primary' onclick='saveProvider("web")'>保存</button><button class='btn' onclick="$('web-form').innerHTML=''">取消</button></div><p class='hint'>自定义网页可以保存登录状态，但只有已实现 Adapter 的网站可以实际对话。</p></div>`;else if(kind==='local')body=`<div class='card'><h2 class='section-title' style='font-size:18px'>${id?'编辑':'新增'}本地模型</h2><div class='form'><label class='field'><span>别名</span><input id='f-id' value='${esc(p.id)}' placeholder='ollama-main'></label><label class='field'><span>显示名称</span><input id='f-name' value='${esc(p.name)}' placeholder='我的本地模型'></label><label class='field full'><span>接口地址</span><input id='f-endpoint' value='${esc(p.endpoint)}' placeholder='http://127.0.0.1:11434/v1'></label><label class='field'><span>模型名称</span><input id='f-model' value='${esc(p.model)}' placeholder='llama3.2'></label><label class='field'><span>最多等待秒数</span><input id='f-timeout' type='number' min='1' value='${p.queue_timeout||30}'></label></div><div class='toolbar'><button class='btn primary' onclick='saveProvider("local")'>保存</button><button class='btn' onclick="$('local-form').innerHTML=''">取消</button></div></div>`;else body=`<div class='card'><h2 class='section-title' style='font-size:18px'>${id?'编辑':'新增'} API 模型</h2><div class='form'><label class='field'><span>别名</span><input id='f-id' value='${esc(p.id)}' placeholder='openai-main'></label><label class='field'><span>显示名称</span><input id='f-name' value='${esc(p.name)}' placeholder='GPT API'></label><label class='field full'><span>API 地址</span><input id='f-endpoint' value='${esc(p.endpoint)}' placeholder='https://api.openai.com/v1'></label><label class='field'><span>模型名称</span><input id='f-model' value='${esc(p.model)}' placeholder='gpt-5.5'></label><label class='field'><span>请求类型</span><select id='f-type'><option value='openai-compatible'>OpenAI 兼容聊天</option><option value='custom-compatible'>自定义兼容接口</option></select></label><label class='field full'><span>API Key（留空保持原密钥）</span><input id='f-key' type='password' placeholder='不会回显旧密钥'></label></div><div class='toolbar'><button class='btn' onclick='testApiForm()'>测试接口</button><button class='btn primary' onclick='saveProvider("api")'>保存</button><button class='btn' onclick="$('api-form').innerHTML=''">取消</button></div></div>`;$(kind+'-form').innerHTML=body;if(kind==='api'&&p.request_type)$('f-type').value=p.request_type}
async function saveProvider(kind){try{const p={id:$('f-id').value,name:$('f-name').value,enabled:true};if(kind==='web')Object.assign(p,{base_url:$('f-url').value,conversation:{supports_flash:$('f-flash').checked,supports_expert:$('f-expert').checked,supports_hybrid:$('f-hybrid').checked,supports_images:$('f-images').checked,create_if_missing:$('f-create').checked,titles:{flash:$('f-flash-title').value,expert:$('f-expert-title').value,hybrid:$('f-hybrid-title').value}}});else Object.assign(p,{endpoint:$('f-endpoint').value,model:$('f-model').value});if(kind==='local')p.queue_timeout=$('f-timeout').value;if(kind==='api'){p.api_key=$('f-key').value;p.request_type=$('f-type').value}const d=await api('/api/providers',{method:'POST',body:JSON.stringify({kind,provider:p})});state=d.config;render();$(kind+'-form').innerHTML='';toast('配置已加密保存')}catch(e){toast(e.message)}}
async function saveCommander(){try{const d=await api('/api/settings',{method:'POST',body:JSON.stringify({commander:{enabled:$('commander-enabled').checked,terminal:$('terminal').value,command:$('command').value,max_agents:Math.max(1,Math.min(5,Number($('max-agents').value)||5))}})});state=d.config;render();toast('Commander 配置已保存')}catch(e){toast(e.message)}}
            ...
            try:
                ...
                if self.path == "/api/environment":
                    checks = [
                        ...
                        {"name": "Codex CLI", "ok": bool(shutil.which("codex")), "detail": shutil.which("codex") or "PATH 中未找到"},
                        {"name": "AgentRelay 虚拟环境", "ok": VENV_PYTHON.is_file(), "detail": str(VENV_PYTHON)},
                        {"name": "Skill 文件", "ok": (TARGET_SKILL / "SKILL.md").is_file(), "detail": str(TARGET_SKILL)},
                        {"name": "Hook 配置", "ok": HOOKS_JSON.is_file(), "detail": str(HOOKS_JSON)},
                    ]
                ...
                if self.path == "/api/settings":
                    ...
                    if isinstance(incoming,dict):
                        config.setdefault("commander",{}).update({"enabled":bool(incoming.get("enabled",False)),"terminal":str(incoming.get("terminal","codex")),"command":str(incoming.get("command","codex exec")),"max_agents":max(1,min(5,int(incoming.get("max_agents",5))))})
