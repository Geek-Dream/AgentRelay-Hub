...
Commander 子 Agent 的求援约束：同一个网页 Provider 会话、同一个本地模型服务全局只能被一个角色占用；角色首次选定的网页 Provider 默认保持黏性，后续同角色任务继续优先使用它，只有 Cookie/代理熔断后才切换。子 Agent 只能通过受控 `commander_provider_call.py` 使用网页或本地 Provider，不能调用 GPT/API Provider，不能再启动 Commander。Cookie/session 失效或代理错误会熔断并最多通知用户三次，通知包含重新登录脚本位置和代理提示；熔断期间仍使用可用备用 Provider。恢复认证后，主 Commander 可以用同一道问题取得原 Provider 和备用 Provider 的回答，按准确性、完整性、可执行性和无效内容进行比较，把更好的 Provider、比较结果和备用期间的上下文交接给后续任务。

模型切换必须围绕当前真实任务进行，不能为了比较而另造无关问题。恢复的 Provider 和备用
Provider 应在恢复阶段分别重新回答原始任务（例如“下载并配置 Redis”），再由主 Agent 按准确性、
完整性、可执行性和无效内容进行判断，并记录两份回答、分数、最终选择和备用期间的上下文摘要。
如果备用 Provider 当时忙或不可用，则保留熔断期间同一任务的回答，并在比较记录中写明原因，不能
伪造一次成功的比较。

本地模型是单并发资源：多个 Agent 请求本地模型时最多只留一个等待者。若线上 Provider 有空闲，
应立即改用线上 Provider；只有候选 Provider 都忙时才等待，等待超过配置时限后再失败或切换，
不能为了并发而启动多个本地模型进程。线上 Provider 也遵守各自单会话占用和最多一个等待者。
...
如果一个角色已经完成，而另一个角色仍在处理、超时或达到自己的升级条件，Commander 可以让这个
空闲角色执行一轮协助任务。协助角色保留原角色、原任务结果和自己的记录，只作为分析、验证或
补充修改者；目标角色仍然是主力，不会被协助角色替代。
...
确认卡的交互优先使用宿主提供的勾选/选择控件；宿主没有控件时，显示文本确认卡并等待用户
明确回复。`确认`、`同意`、`好的`、`没问题`、`可以` 等明确肯定回复可以视为批准，`取消`、
`拒绝`、`不要` 等回复视为拒绝。含糊的“嗯”只有在上下文明确指向当前确认卡时才可批准，不能
因为普通闲聊或单独提到模型名称而执行任务。
